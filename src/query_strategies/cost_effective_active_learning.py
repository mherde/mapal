...
from src.models.nn_annotators_model import NNAnnotatorsModel
...
class CEAL(QueryStrategy):
    ...
    nn_annots_mdl: NNAnnotatorsModel
        The annotators model to estimate the sample-dependent annotation performances of the annotators.
    ...
    def __init__(self, **kwargs):
        ...
        n_neighbors = int(kwargs.pop('n_neighbors', 10))
        metric = kwargs.pop('metric', 'rbf')
        self.nn_annots_mdl_ = NNAnnotatorsModel(n_classes=self.n_classes_, n_neighbors=n_neighbors, metric=metric,
                                                random_state=deepcopy(self.random_state_), **kwargs)
    ...
    def compute_scores(self):
        ...
        if len(fully_labeled_indices) < self.min_samples_:
            ...
            uncertainties = uncertainty_scores(P=P, method='lc').reshape(-1, 1)
            scores[unlabeled_indices] = uncertainties

            if self.annot_perfs_ is None:
                # retrain annotators model
                self.nn_annots_mdl_.fit(X=self.data_set_.X_[fully_labeled_indices],
                ...
                self.annot_perfs_ = self.nn_annots_mdl_.predict(self.data_set_.X_)
